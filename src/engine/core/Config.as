	...
	public class Config {
		...
		private static function myContext3DHandler(evt : Event) : void {
			...
			if(Scene_data.context3D.driverInfo.indexOf("Software") != -1 && Boolean(softModeFunction)){
				if(Scene_data.context3D.driverInfo == "Software Hw_disabled=userDisabled"){
					softModeFunction(1);
				}else if(Scene_data.context3D.driverInfo == "Software Hw_disabled=oldDriver" && Scene_data.context3D.driverInfo == "Software Hw_disabled=unavailable"){
					softModeFunction(2);
				}else{
					softModeFunction(3);
				}
